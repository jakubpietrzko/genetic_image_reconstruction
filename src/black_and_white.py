...
def load_target_image(image_path, target_size):
    target_image = Image.open(image_path).convert('L')  # Wczytanie obrazu jako czarno-biały
...
def create_individual(image_size):
    ...
    individual_array = torch.randint(0, 256, size=(image_size[0], image_size[1]), dtype=torch.uint8).cuda()
...
def mutate(population, mutation_rate=0.01):
    ...
    mutated_population = torch.where(mutation_mask, mutation_values, population)
...
def genetic_algorithm(target_image, generations=100, population_size=1000, mutation_rate=0.01, resume=False):
    ...
    # Zmienne do przechowywania parametrów kary
    generation_data = []
    ...
    for generation in range(generations):
        ...
        fitness_scores = fitness_v0(population, target_image)
        ...
        generation_data.append({
            ...
            'best_fitness': fitness_scores[best_idx].item(),
    ...
    # Zapisanie danych do analizy
    np.save('generation_data.npy', generation_data)
